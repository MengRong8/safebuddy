...
const String ESP_IP = '192.168.0.105';
...
class _SafeBuddyHomePageState extends State<SafeBuddyHomePage>
  ...
  void initState() {
    ...
    // }
    _httpService = HttpService(espIp: ESP_IP);
    _startHttpListener(); // 啟動監聽
...
// [新增] HTTP 監聽函式 (取代原本的 _startSerialListener)
  void _startHttpListener() {
    if (_httpService == null) return;

    // 1. 監聽連線狀態
    _httpService!.connectionStatusStream.listen((isConnected) {
      if (mounted) {
        setState(() {
          _isBleConnected = isConnected;
          // UI 文字可以稍微改一下，讓自己知道現在是用 Wi-Fi
          _bleStatus = isConnected ? 'Wi-Fi 已連線' : '未連線';
        });
      }
    });

    // 2. 啟動輪詢 (開始每 0.5 秒問一次 ESP32)
    _httpService!.startPolling();

    // 3. 接收按鈕訊號
    _httpService!.dataStream.listen((line) {
      // 收到 "pressed" -> 觸發警報
      if (line == 'pressed') {
        print('>>> [HTTP] 收到觸發訊號');
  ...
  // [新增] HTTP 重連函式 (取代原本的 _retryConnection)
  Future<void> _retryHttpConnection() async {
    if (_httpService == null) return;
    ...
    if (mounted) {
       ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('正在重新連接 Wi-Fi...'), backgroundColor: Colors.orange),
      );
    }
  }
